...
SELECT	Id,
		UserId,
		Score,
		RANK() OVER (PARTITION BY UserID ORDER BY Score DESC) ScoreRank
FROM	dbo.Comments_Heap
)
SELECT	TOP 100 
		c.UserId,
		Text
FROM	dbo.Comments_Heap c
INNER	JOIN ScoreRank sc ON c.Id = sc.Id
WHERE sc.ScoreRank > 10
